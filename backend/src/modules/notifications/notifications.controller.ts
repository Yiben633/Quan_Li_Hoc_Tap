...
export async function cron(req: Request, res: Response) { try { const bearer = req.header('authorization')?.replace(/^Bearer\s+/i, ''); const provided = req.header('x-cron-secret') ?? bearer; if (!env.CRON_SECRET || provided !== env.CRON_SECRET) return sendError(res, 'Invalid cron secret', undefined, 401); return sendSuccess(res, 'Notifications processed', await service.runNotificationEngine()); } catch (e) { return handle(res, e); } }
