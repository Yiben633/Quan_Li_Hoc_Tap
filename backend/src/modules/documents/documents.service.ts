...
function storageKey(url: string) { return url.replace(/^\/uploads\//, ''); }
...
export async function upload(userId: string, file: Express.Multer.File, input: { title?: string; subjectId?: string | null; taskId?: string | null; tags: string[] }, context?: Context) {
  ...
  try {
    ...
    const item = await prisma.document.create({ data: { userId, subjectId: input.subjectId ?? null, taskId: input.taskId ?? null, title: input.title || file.originalname, fileUrl: stored.url, fileType: fileTypeForMime(file.mimetype), storageProvider: 'local', sizeBytes: file.size, tags: input.tags } });
...
export async function remove(userId: string, id: string, context?: Context) { const item = await owned(userId, id); await storageProvider.delete(storageKey(item.fileUrl)); await prisma.document.delete({ where: { id } }); await log(userId, 'document.deleted', id, context); return { id }; }
