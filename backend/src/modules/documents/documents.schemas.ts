...
export const listSchema = z.object({ semesterId: z.string().uuid().optional(), subjectId: z.string().uuid().optional(), taskId: z.string().uuid().optional(), tag: z.string().trim().max(50).optional(), fileType: z.enum(['pdf', 'word', 'excel', 'ppt', 'image', 'link', 'video', 'code', 'other']).optional(), search: z.string().trim().max(100).optional(), page: z.coerce.number().int().min(1).default(1), limit: z.coerce.number().int().min(1).max(100).default(20) });
