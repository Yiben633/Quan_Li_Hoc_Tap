...
export async function invitations(req: Request, res: Response) { try { return sendSuccess(res, 'Pending invitations fetched', await s.invitations(req.user!.id)); } catch (e) { return h(res, e); } }
...
export async function invite(req: Request, res: Response) { try { return sendSuccess(res, 'Member invited', await s.invite(req.user!.id, p(req, 'id'), req.body.email), 201); } catch (e) { return h(res, e); } }
...
export async function reject(req: Request, res: Response) { try { return sendSuccess(res, 'Invitation declined', await s.reject(req.user!.id, p(req, 'id'), p(req, 'memberId'))); } catch (e) { return h(res, e); } }
