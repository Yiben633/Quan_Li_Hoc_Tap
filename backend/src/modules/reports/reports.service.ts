...
async function report(userId: string, start: Date, end: Date, subjectId?: string) {
  if (subjectId) await subjectOwned(userId, subjectId);
  ...
  const [tasks, sessions, subjects, overdue] = await Promise.all([
    ...
    prisma.subject.findMany({ where: { ...where, deletedAt: null }, select: { id: true, code: true, name: true, credits: true } }),
  ...
  return { start, end, subjectId: subjectId ?? null, taskDone: tasks.length, overdueTasks: overdue, totalStudyMinutes: totalMinutes, totalStudyHours: Math.round(totalMinutes / 60 * 100) / 100, sessions, subjects, tasks };
}
...
export async function weekly(userId: string) { const r = range('weekly'); return report(userId, r.start, r.end); }
export async function monthly(userId: string) { const r = range('monthly'); return report(userId, r.start, r.end); }
export async function semester(userId: string, semesterId: string) { const item = await prisma.semester.findFirst({ where: { id: semesterId, userId, deletedAt: null } }); if (!item) throw serviceError('Semester not found', 404); return { semester: item, ...(await report(userId, item.startDate, addDays(item.endDate, 1))) }; }
export async function bySubject(userId: string, subjectId: string) { const subject = await subjectOwned(userId, subjectId); const end = addDays(startOfDay(), 1); return { subject, ...(await report(userId, subjectId ? startOfDay(new Date(subject.createdAt)) : startOfDay(), end, subjectId)) }; }
export async function exportReport(userId: string, format: 'pdf' | 'excel', type: 'weekly' | 'monthly' = 'weekly') { const data = type === 'monthly' ? await monthly(userId) : await weekly(userId); if (format === 'excel') { const workbook = new ExcelJS.Workbook(); const sheet = workbook.addWorksheet('Report'); sheet.addRows([['StudyFlow Report', type], ['Start', data.start.toISOString()], ['End', data.end.toISOString()], ['Tasks done', data.taskDone], ['Overdue tasks', data.overdueTasks], ['Study minutes', data.totalStudyMinutes]]); return { buffer: Buffer.from(await workbook.xlsx.writeBuffer()), contentType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', fileName: `studyflow-${type}.xlsx` }; } const chunks: Buffer[] = []; const document = new PDFDocument(); document.on('data', (chunk: Buffer) => chunks.push(chunk)); const finished = new Promise<void>((resolve) => document.on('end', resolve)); document.fontSize(20).text(`StudyFlow ${type} report`); document.moveDown().fontSize(12).text(`Tasks done: ${data.taskDone}`).text(`Overdue tasks: ${data.overdueTasks}`).text(`Study minutes: ${data.totalStudyMinutes}`); document.end(); await finished; return { buffer: Buffer.concat(chunks), contentType: 'application/pdf', fileName: `studyflow-${type}.pdf` }; }
