...
export async function uploadAvatar(file: File) { const body = new FormData(); body.append('avatar', file, file.name || 'avatar.png'); const response = await apiClient.patch<ApiResponse<AuthUser>>('/users/me/avatar', body, { headers: { 'Content-Type': undefined } }); return response.data.data }
