...
export async function login(input: LoginValues) { const response = await apiClient.post<ApiResponse<{ user: AuthUser; accessToken: string; csrfToken: string }>>('/auth/login', { email: input.email, password: input.password }); return response.data.data }
