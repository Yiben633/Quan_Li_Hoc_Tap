...
export function getApiErrorMessage(error: unknown, fallback = 'Có lỗi xảy ra. Vui lòng thử lại.') { const typed = error as { code?: string; message?: string; response?: { data?: { message?: string; errors?: Array<{ message?: string }> } } }; const response = typed.response?.data; if (!response && (typed.code === 'ERR_NETWORK' || typed.message?.toLowerCase().includes('network'))) return 'Không thể kết nối máy chủ. Hãy kiểm tra backend đang chạy tại cổng 4000.'; return response?.message ?? response?.errors?.[0]?.message ?? fallback }
