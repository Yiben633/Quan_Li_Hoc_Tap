...
export function TaskRow({ task, mode = 'default', selectionMode = false, selected = false, onSelect, onOpen, onEdit, onDuplicate, menuOpen, onMenuOpenChange, onStatusChange, onDelete }: TaskRowProps) {
  ...
  const context = [task.subject?.name, task.studyPlan?.title].filter(Boolean)
  ...
  return <article className={`task-list-row task-status-${task.status}`}>
    ...
    <div className="task-row-content">
      <div className="task-row-primary"><button type="button" className="task-row-main" onClick={onOpen}><span className="task-row-title">{task.title}</span></button><span className={`status-label task-row-status task-pill-${task.status}`}>{TASK_STATUS_LABELS[task.status]}</span></div>
      {context.length > 0 && <p className="task-row-context">{context.join(' · ')}</p>}
    ...
    <Dropdown label={<><MoreHorizontal size={18} /><span className="sr-only">Thao tác với {task.title}</span></>} open={menuOpen} onOpenChange={onMenuOpenChange} showChevron={false}>
