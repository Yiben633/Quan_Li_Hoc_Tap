...
import { Button, Checkbox, Drawer, EmptyState, Skeleton } from '../../../components/ui'
import { formatTaskDate, formatTaskDeadline } from '../../../utils/taskDate'
import { useTopicsQuery } from '../../learning/learning.hooks'
import { useCreateSubtaskMutation, usePlansQuery, useSubtaskMutation, useTaskAttachmentMutation, useTaskDuplicateMutation, useTaskQuery, useTaskUpdateMutation } from '../tasks.hooks'
...
export function TaskDrawer({ id, onClose, onDelete }: { id: string; onClose: () => void; onDelete: (id: string) => void }) {
  ...
  return <><Drawer open={Boolean(id)} title={task?.title ?? 'Chi tiết công việc'} onClose={onClose}><div className="task-drawer-content">{query.isLoading ? <Skeleton height={420} /> : query.isError || !task ? <EmptyState title="Không thể tải công việc" description="Thử đóng rồi mở lại." /> : <>
    ...
    <section className="drawer-section"><div className="drawer-section-heading"><div><h3>Checklist</h3><span className="drawer-section-count">{task.subTasks.filter((subtask) => subtask.isDone).length} / {task.subTasks.length}</span></div></div>{task.subTasks.length ? task.subTasks.map((subtask) => <Checkbox key={subtask.id} label={subtask.title} checked={subtask.isDone} onChange={(event) => subtaskMutation.mutate({ taskId: task.id, subtaskId: subtask.id, isDone: event.target.checked })} />) : <p className="subtle">Chưa có việc nhỏ.</p>}<form className="drawer-subtask-form" onSubmit={addSubtask}><input value={subtaskTitle} onChange={(event) => setSubtaskTitle(event.target.value)} placeholder="Thêm việc nhỏ..." aria-label="Tên việc nhỏ" /><Button type="submit" variant="secondary" loading={createSubtaskMutation.isPending} disabled={!subtaskTitle.trim()}><Plus size={15} /> Thêm việc nhỏ</Button></form></section>
