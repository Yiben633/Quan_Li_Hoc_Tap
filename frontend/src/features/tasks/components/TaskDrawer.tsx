import { CalendarDays, Clock3, Copy, Edit3, Gauge, Paperclip, Plus, Sparkles, Trash2 } from 'lucide-react'
...
import { StartStudyButton } from '../../study-sessions/StartStudyButton'
...
export function TaskDrawer({ id, onClose, onDelete }: { id: string; onClose: () => void; onDelete: (id: string) => void }) {
  ...
  return <><Drawer open={Boolean(id)} title={task?.title ?? 'Chi tiết công việc'} onClose={onClose}><div className="task-drawer-content">{query.isLoading ? <Skeleton height={420} /> : query.isError || !task ? <EmptyState title="Không thể tải công việc" description="Thử đóng rồi mở lại." /> : <>
    <div className="drawer-task-status"><span className={`status-label task-pill-${task.status}`} aria-label={`Trạng thái: ${TASK_STATUS_LABELS[task.status]}`}>{TASK_STATUS_LABELS[task.status]}</span><span className={`priority priority-${task.priority}`} aria-label={`Ưu tiên: ${PRIORITY_LABELS[task.priority]}`}>{PRIORITY_LABELS[task.priority]}</span></div>
    ...
    <div className="drawer-actions">{task.subjectId && <StartStudyButton subjectId={task.subjectId} taskId={task.id} label="Bắt đầu" />}<Link className="button secondary" to={`/ai-coach?taskId=${encodeURIComponent(task.id)}`} onClick={onClose}><Sparkles size={15} /> Hỏi AI cách sắp xếp việc này</Link><Button variant="secondary" onClick={() => setEditOpen(true)}><Edit3 size={15} /> Chỉnh sửa</Button><Button variant="secondary" onClick={() => duplicate.mutate(task.id, { onSuccess: () => toast.success('Đã nhân bản công việc'), onError: () => toast.error('Không thể nhân bản công việc') })}><Copy size={15} /> Nhân bản</Button><Button variant="danger" onClick={() => onDelete(task.id)}><Trash2 size={15} /> Xóa</Button></div>
