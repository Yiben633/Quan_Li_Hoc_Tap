  ...
  return <div className="ai-schedule-layout">
    ...
    <section className="panel ai-preview" aria-live="polite">
      ...
      {!suggestion ? <EmptyState icon={<Clock3 size={23} />} title="Chưa có lịch gợi ý" description="Thêm công việc và khung giờ để tạo bản xem trước." /> : <>
        ...
        {suggestion.warnings.length > 0 && <div className="ai-warning" role="alert"><AlertTriangle size={18} /><div><strong>Khung giờ hiện chưa đủ</strong><p>{suggestion.warnings.length} công việc chưa thể xếp. Hãy thêm thời gian rảnh hoặc giảm thời lượng.</p></div></div>}
