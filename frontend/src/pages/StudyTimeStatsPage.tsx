...
import { Button, ChartLegend, EmptyState, ErrorState, Select, Skeleton } from '../components/ui'
...
export function StudyTimeStatsPage() {
  ...
  return <div className="study-stats-page">
    ...
    {query.isLoading ? <Skeleton height={320} /> : query.isError ? <ErrorState title="Không thể tải thời gian học." action={<Button onClick={() => void query.refetch()}>Thử lại</Button>} /> : !query.data?.sessionCount ? <EmptyState icon={<NatureEmptyMascot kind="focus" size={104} className="study-stats-empty-bear" />} title="Chưa có phiên học trong khoảng này" description="Khi bạn hoàn thành một phiên tập trung, thống kê sẽ xuất hiện ở đây." /> : <><section className="study-stats-summary"><article><span>Tổng thời gian</span><strong>{query.data.totalHours} giờ</strong></article><article><span>Số phiên</span><strong>{query.data.sessionCount}</strong></article><article><span>Trung bình mỗi phiên</span><strong>{Math.round(query.data.totalMinutes / query.data.sessionCount)} phút</strong></article></section><section className="panel study-time-chart"><div><h2>Thời gian đã ghi nhận</h2><p className="subtle">Phút theo ngày trong {rangeLabels[range].toLowerCase()}.</p></div><ChartLegend items={[{ label: 'Thời gian học', tone: 'primary' }]} /><div className="study-time-chart-wrap"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{ top: 16, right: 8, left: -18, bottom: 0 }}><CartesianGrid vertical={false} stroke="var(--line)" /><XAxis dataKey="date" tickLine={false} axisLine={false} /><YAxis tickLine={false} axisLine={false} allowDecimals={false} /><Tooltip cursor={{ fill: 'var(--blue-soft)' }} formatter={(value: number) => [`${value} phút`, 'Thời gian']} /><Bar dataKey="minutes" name="Thời gian" fill="var(--blue)" radius={[5, 5, 0, 0]} /></BarChart></ResponsiveContainer></div></section></>}
  </div>
}
