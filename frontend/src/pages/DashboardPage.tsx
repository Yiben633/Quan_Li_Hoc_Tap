...
function formatChartDate(date: string, range: Range) { const parts = new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Ho_Chi_Minh', weekday: 'short', day: '2-digit', month: '2-digit' }).formatToParts(new Date(date)); const values = Object.fromEntries(parts.map((part) => [part.type, part.value])); if (range === 'month') return `${values.day}/${values.month}`; const weekdays: Record<string, string> = { Mon: 'T2', Tue: 'T3', Wed: 'T4', Thu: 'T5', Fri: 'T6', Sat: 'T7', Sun: 'CN' }; return `${weekdays[values.weekday] ?? values.weekday} ${values.day}/${values.month}` }
