...
export function DashboardPage() {
  ...
  return (
    <div className="dashboard">
      ...
      <section className="panel quick-panel"><div className="panel-heading"><div><h2>Bắt đầu nhanh</h2><p className="subtle">Chọn một việc để tiếp tục</p></div><MoreHorizontal size={18} className="panel-icon" /></div><div className="quick-actions"><Link className="quick-action" to="/goals"><Target size={17} /><span><strong>Tạo mục tiêu</strong><small>Chọn điều muốn tiến bộ</small></span></Link><Link className="quick-action" to="/study-plans"><Check size={17} /><span><strong>Tạo kế hoạch</strong><small>Chia nhỏ các bước</small></span></Link><Link className="quick-action" to="/tasks"><Check size={17} /><span><strong>Tạo việc</strong><small>Chia nhỏ việc cần làm</small></span></Link><Link className="quick-action" to="/calendar"><CalendarDays size={17} /><span><strong>Mở lịch</strong><small>Sắp xếp thời gian</small></span></Link><Link className="quick-action" to="/study"><Clock3 size={17} /><span><strong>Tập trung</strong><small>Bắt đầu phiên học</small></span></Link><Link className="quick-action" to="/kanban"><Columns3 size={17} /><span><strong>Mở Kanban</strong><small>Điều phối tiến độ</small></span></Link></div></section>
    </div>
  )
}
...
function EventRow({ event }: { event: DashboardSchedule }) { const dateParts = event.startDate ? formatDayMonth(event.startDate).split('/') : []; const isTask = event.type === 'task_due'; return <div className={`event-row${isTask ? ' event-row-task' : ''}`}><div className="event-date blue"><strong>{dateParts[0] ?? '•'}</strong><span>{dateParts[1] ? `THÁNG ${dateParts[1]}` : 'SẮP TỚI'}</span></div><div><strong>{event.title}</strong><p className="subtle">{isTask ? 'Hạn công việc' : `${event.startTime}${event.endTime ? ` - ${event.endTime}` : ''}`}</p></div></div> }
