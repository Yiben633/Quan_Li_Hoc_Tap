import { ArrowUpRight, CalendarDays, Check, Clock3, Columns3, MoreHorizontal, Paperclip, Plus, Sparkles, Target, TrendingUp, X } from 'lucide-react'
...
import type { DashboardGoal, DashboardSchedule, DashboardSubject, DashboardSummary, DashboardTask } from '../features/dashboard/dashboard.api'
...
export function DashboardPage() {
  ...
  return (
    <div className="dashboard">
      ...
      {data.dailyBriefing && <DailyBriefing briefing={data.dailyBriefing} />}
...
function DailyBriefing({ briefing }: { briefing: NonNullable<DashboardSummary['dailyBriefing']> }) { return <section className="panel daily-briefing"><header><span className="daily-briefing-icon" aria-hidden="true"><Sparkles size={18} /></span><div><p>AI COACH - HÔM NAY</p><h2>Nhịp học của bạn</h2></div></header><div className="daily-briefing-copy"><span>Bạn có <strong>{briefing.openTaskCount} công việc</strong> đang mở.</span><span>{briefing.dueTodayCount > 0 ? <><strong>{briefing.dueTodayCount} việc</strong> đến hạn hôm nay.</> : 'Không có việc nào đến hạn hôm nay.'}</span><span>{briefing.availableSlot ? <>Khoảng trống: <strong>{briefing.availableSlot.startTime} - {briefing.availableSlot.endTime}</strong>.</> : 'Chưa tìm thấy khoảng trống ít nhất 30 phút trong hôm nay.'}</span></div><Link className="button secondary daily-briefing-action" to="/ai-coach?prompt=Gợi%20%C3%BD%20l%E1%BB%8Bch%20h%E1%BB%8Dc%20h%C3%B4m%20nay%20d%E1%BB%B1a%20tr%C3%AAn%20c%C3%B4ng%20vi%E1%BB%87c%20v%C3%A0%20l%E1%BB%8Bch%20tr%E1%BB%91ng%20c%E1%BB%A7a%20t%C3%B4i."><Sparkles size={15} /> Gợi ý lịch hôm nay</Link></section> }
