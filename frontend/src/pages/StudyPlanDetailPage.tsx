...
export function StudyPlanDetailPage() {
  ...
  if (planQuery.isError || !plan) return <EmptyState icon={<Target size={24} />} title="Không thể tải kế hoạch" description="Kế hoạch có thể đã bị xóa hoặc kết nối đang gặp sự cố." action={<Button onClick={() => void planQuery.refetch()}>Thử lại</Button>} />
